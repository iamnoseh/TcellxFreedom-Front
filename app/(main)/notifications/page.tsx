          ...
          notifications.map((notif) => (
            <Card key={notif.id} variant="default" className="flex gap-3">
              <div className="w-10 h-10 rounded-xl bg-purple-100 flex items-center justify-center flex-shrink-0">
                <Bell size={18} className="text-[#7B2FBE]" />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-gray-400 text-xs">{notif.taskTitle}</span>
                  <span className="text-gray-300 text-xs">•</span>
                  <span className="text-gray-400 text-xs">{formatTime(notif.scheduledAt)}</span>
                </div>
              </div>
            </Card>
          ))
